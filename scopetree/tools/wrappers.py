...
class Crtsh(BaseTool):
    """Crtsh wrapper - Certificate Transparency logs subdomain enumeration"""
    ...
    async def run(self, domain: str) -> List[str]:
        stdout, _, _ = await self.run_tool(
            [self.command, '-s', f"https://crt.sh/?q=%.{domain}&output=json"]
        )
        ...
        subdomains = {
            ...
            if s.strip() and '*' not in s and (s.strip().lower().endswith(f'.{domain}') or s.strip().lower() == domain)
        }
